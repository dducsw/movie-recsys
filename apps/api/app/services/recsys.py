...
def get_ranker_model():
    ...
    try:
        ...
        candidates = [
            os.path.join(repo_root, "pipelines", "training", "ml_pipeline", "models", "lgb_ranker.pkl"),
            os.path.join(repo_root, "models", "lgb_ranker.pkl"),
            os.path.join(repo_root, "evaluation", "ml_pipeline", "models", "lgb_ranker.pkl"),
        ]
        ...
        if not os.path.exists(model_path):
            ...
            res = requests.get(seaweed_url, timeout=5.0)
            if res.status_code == 200:
                content = res.content
                # Verify SHA256 integrity if header or env specifies expected hash
                expected_sha = res.headers.get("X-Checksum-SHA256") or os.getenv("RANKER_MODEL_SHA256")
                if expected_sha:
                    import hashlib
                    actual_sha = hashlib.sha256(content).hexdigest()
                    if actual_sha.lower() != expected_sha.strip().lower():
                        raise ValueError(f"Checksum mismatch for ranker model! Expected {expected_sha}, got {actual_sha}")
...
class RecsysService:
    ...
    def _stage_2_ranking(
        ...
        for cand in candidates:
            ...
            feat = {
                ...
                "user_activity": float(np.log1p(liked_count)),
                "user_bias": user_bias,
                "als_score": retrieval_score,
                "cb_score": float(genre_overlap / max(len(target_genres), 1))
            }
    ...
    def get_cold_start_recommendations(
        ...
        """
        Public API: Cold-start recommendation strategy for users with zero interaction history.
        Prioritizes high-popularity items matching preferred onboarding genres if provided;
        falls back to global trending items across the catalog.
        """
        genre_key = ",".join(sorted(preferred_genres or []))
        cache_key = f"cold_start:{genre_key}:{limit}"
        cached = recsys_cache.get(cache_key)
        if cached is not None:
            RECSYS_CACHE_HITS.labels(cache_type="cold_start").inc()
            return cached

        RECSYS_CACHE_MISSES.labels(cache_type="cold_start").inc()
        trending = MovieModel.get_trending(page=1, limit=limit * 3)

        if not preferred_genres:
            res = trending[:limit]
        else:
            pref_set = set(g.strip().lower() for g in preferred_genres)
            matched = []
            unmatched = []
            for m in trending:
                m_genres = set(g.strip().lower() for g in str(m.get("genres", "")).split("|"))
                if pref_set.intersection(m_genres):
                    matched.append(m)
                else:
                    unmatched.append(m)
            res = (matched + unmatched)[:limit]

        recsys_cache.set(cache_key, res, ttl_seconds=300)
        RECSYS_REQUESTS_TOTAL.labels(endpoint="cold_start_recommendations", status="success").inc()
        return res
