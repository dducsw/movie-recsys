...
# Note: Never use bare short ambiguous substrings like "ma" which match "mà", "mang", "tham gia"
GENRE_MAP = {
...
def detect_user_language(text: str) -> str:
    ...
    vi_tokens = {
        ...
        "hoạt", "hình", "chào", "bạn", "giúp", "ơi", "nhé", "ạ", "cảm", "ơn", "mà"
    }
...
def heuristic_detect_intent(user_text: str) -> IntentOutput:
    """Fast, deterministic fallback intent classifier for RecSys when LLM is offline."""
    text_lower = user_text.lower().strip()
    lang = detect_user_language(user_text)

    # 1. Check for Similar Movie Intent
    similar_patterns = [
        r"(?:similar to|like|movies like|films like|resemble|tương tự|giống phim|giống như)\s+['\"]?([^'\"?.,!]+)['\"]?",
    ...
    # 2. Check for Genre Intent with Word Boundary Matching
    found_genres: List[str] = []
    for keyword, canonical_genre in GENRE_MAP.items():
        # Match whole word / boundary only to avoid substring collisions
        if re.search(r'(?:\b|_)' + re.escape(keyword) + r'(?:\b|_)', text_lower):
            if canonical_genre not in found_genres:
                found_genres.append(canonical_genre)

    if found_genres:
        return IntentOutput(intent="genre", target_title="", genres=found_genres, language=lang)

    # 3. Check for Personalized / General Recommendation Intent
    personal_keywords = [
    ...
    if any(k in text_lower for k in ["first", "second", "third", "thứ nhất", "thứ hai", "bộ đó", "phim đó", "cốt truyện", "diễn viên"]):
...
def detect_intent_node(state: GraphState) -> dict:
    """Classify user intent using fast deterministic detector, falling back to LLM for complex queries."""
    messages = state.get("messages", [])
    last_text = messages[-1].content if messages else ""
    lang = detect_user_language(last_text)

    # 1. Fast deterministic check (< 1ms, 100% accurate for genres and explicit movie titles)
    heuristic_res = heuristic_detect_intent(last_text)
    if heuristic_res.intent != "fallback":
        heuristic_res.language = lang
        logger.info(f"Heuristic intent detected: {heuristic_res.intent}, genres: {heuristic_res.genres}, lang: {lang}")
        return {"intent_output": heuristic_res, "detected_language": lang}

    # 2. If ambiguous or chitchat, consult LLM
    llm = get_llm()
    if llm is not None:
        try:
