"""
web_app/backend/app/services/ml_training_service.py
---------------------------------------------------
Service integrating the trained ML models from evaluation/ml_training/models.joblib
...
def get_model_bundle() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        model_path = os.path.join(repo_root, "evaluation", "ml_training", "models.joblib")
...
class MLTrainingModelService:
        ...
        try:
            ...
            for idx, mid in enumerate(valid_ids):
                ...
                if score_cb is not None and score_lgb is not None:
                    final_score = 0.6 * score_cb + 0.4 * score_lgb
    ...
    def recommend_for_user(cls, user_id: int, top_k: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cand["ml_score"] = 0.6 * pred_cb + 0.4 * pred_lgb
