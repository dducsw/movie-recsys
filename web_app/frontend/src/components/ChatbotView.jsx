...
function ChatbotView({ 
  ...
  const getYear = (dateStr) => {
    if (!dateStr) return '';
    // Handle both '2023' format and '2023-10-15' format
    if (dateStr.length === 4) return dateStr;
    return dateStr.split('-')[0] || '';
  };

  const getPosterUrl = (movie) => {
    // New format uses image_url, old format uses poster_url
  ...
  return (
    <div className="chatbot-container">
      ...
      <div className="chatbot-chatbox">
              ...
              <div className="bubble-content">
                ...
                {msg.movies && msg.movies.length > 0 && (
                  <div className="chat-movie-list">
                    <div className="movie-list-label">🎬 Recommended Movies:</div>
                    {msg.movies.map((movie, movieIndex) => (
                      ...
                      >
                        ...
                        <div className="chat-movie-info">
                          ...
                          {/* Show description if available (from Tavily enrichment) */}
                          {movie.description && (
        ...
        <div className="chatbot-input-container">
          <input
            ...
            placeholder="Ask me anything about movies... (e.g., 'movies like Inception', 'sci-fi action')"
            value={chatInput}
      ...
      <div className="chatbot-chips">
        ...
        <span className="chatbot-chip" onClick={() => handleSendChatMessage("I want to watch family animation movies")}>
          👶 Family Animation
        </span>
        <span className="chatbot-chip" onClick={() => handleSendChatMessage("Find movies similar to Toy Story")}>
          🧸 Like Toy Story
        </span>
        <span className="chatbot-chip" onClick={() => handleSendChatMessage("Recommend thriller horror movies")}>
          👻 Thriller Horror
        </span>
        <span className="chatbot-chip" onClick={() => handleSendChatMessage("What are some good comedy movies?")}>
          😂 Comedy Movies
        </span>
        <span className="chatbot-chip" onClick={() => handleSendChatMessage("Movies like The Dark Knight")}>
          🦇 Like Dark Knight
        </span>
      </div>
    </div>
